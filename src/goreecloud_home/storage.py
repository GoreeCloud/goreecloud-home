...
class SQLiteStateStore:
    ...
    def load(self) -> tuple[dict[str, Home], dict[str, Room], dict[str, Device]]:
        ...
        for row in self.database.fetchall(
            """
            SELECT device_id, state_kind, capability, value_json
            FROM device_state
            ...
            target[str(row["capability"])] = json.loads(str(row["value_json"]))
    ...
    def set_device_state(
        ...
        self.database.execute(
            """
            INSERT INTO device_state(device_id, state_kind, capability, value_json, updated_at)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(device_id, state_kind, capability)
            DO UPDATE SET value_json=excluded.value_json, updated_at=excluded.updated_at
            """,
            (device_id, state_kind, capability, encoded, _utc_now()),
        )
